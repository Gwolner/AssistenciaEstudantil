...
class grid_relatorio_emprestimo_xml
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   //---- 
   function grid_relatorio_emprestimo_xml()
   {
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "grid_relatorio_emprestimo.xml";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_emprestimo']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_emprestimo']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_emprestimo']['xml_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_emprestimo']['xml_name']);
      }
      if (!$this->Grava_view)
      {
   ...
   END as sc_field_1, concat(a.nome_aluno,' ',a.matricula) as sc_field_0 from " . $this->Ini->nm_tabela; 
      ...
      while (!$rs->EOF)
      {
         $this->xml_registro = "<grid_relatorio_emprestimo";
   ...
   function NM_export_e_data_entrega()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->e_data_entrega))
         {
             $this->e_data_entrega = sc_convert_encoding($this->e_data_entrega, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " e_data_entrega =\"" . $this->trata_dados($this->e_data_entrega) . "\"";
   }
   //----- e_data_devolucao
   function NM_export_e_data_devolucao()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->e_data_devolucao))
         {
             $this->e_data_devolucao = sc_convert_encoding($this->e_data_devolucao, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " e_data_devolucao =\"" . $this->trata_dados($this->e_data_devolucao) . "\"";
   }
   //----- e_status
   function NM_export_e_status()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->e_status))
         {
             $this->e_status = sc_convert_encoding($this->e_status, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " e_status =\"" . $this->trata_dados($this->e_status) . "\"";
   }
   //----- sc_field_1
   function NM_export_sc_field_1()
   {
         ...
         $this->xml_registro .= " CASE
       WHEN es.materia = paradidatico THEN es.titulo
       ELSE es.materia
   END =\"" . $this->trata_dados($this->sc_field_1) . "\"";
   }
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_emprestimo']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_emprestimo']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
