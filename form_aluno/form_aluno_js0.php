...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_aluno']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_aluno"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  ...
  {
      foreach ($this->nm_mens_alert as $mensagem)
      {
         echo "alert('" . html_entity_decode($mensagem, ENT_COMPAT, $_SESSION['scriptcase']['charset']) . "');";
      }
  }
?> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_aluno']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_aluno_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_aluno_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
