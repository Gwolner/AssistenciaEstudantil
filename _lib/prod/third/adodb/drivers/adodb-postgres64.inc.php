...
WHERE relkind in ('r','v') AND (c.relname='%s' or c.relname = lower('%s'))
 and c.relnamespace=n.oid and n.nspname='%s'
	...
	FROM pg_class bc, pg_class ic, pg_index i, pg_attribute a, pg_namespace n WHERE bc.oid = i.indrelid AND ic.oid = i.indexrelid AND (i.indkey[0] = a.attnum OR i.indkey[1] = a.attnum OR i.indkey[2] = a.attnum OR i.indkey[3] = a.attnum OR i.indkey[4] = a.attnum OR i.indkey[5] = a.attnum OR i.indkey[6] = a.attnum OR i.indkey[7] = a.attnum) AND a.attrelid = bc.oid AND bc.relnamespace=n.oid AND (n.nspname='%s' OR '%s' = '') AND bc.relname = '%s'";
	...
	var $metaDefaultsSQL = "SELECT d.adnum as num, d.adsrc as def from pg_attrdef d, pg_class c, pg_namespace n where d.adrelid=c.oid AND c.relnamespace=n.oid AND (n.nspname='%s' OR '%s'='') AND c.relname='%s' order by d.adnum";
...
select viewname,'V' from pg_views where viewname like $mask";
	...
	*/
	function BlobDecode($blob,$maxsize=false,$hastrans=true)
	{
		...
		$realblob = @pg_lo_read($fd, 32000000);
		@pg_loclose($fd);
		if ($hastrans) @pg_exec($this->_connectionID,"commit");
		return $realblob;
	}
	...
	*/
	...
	function MetaColumns($table,$normalize=true)
	{
		...
		if (!empty($this->metaKeySQL)) {
			...
			$rskey = $this->Execute(sprintf($this->metaKeySQL, $schema, $schema, $table));
		...
		if (!empty($this->metaDefaultsSQL)) {
			$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
			$sql = sprintf($this->metaDefaultsSQL, $schema, $schema, $table);
...
WHERE (c2.relname=\'%s\' or c2.relname=lower(\'%s\'))';
                ...
                $col_names = $this->MetaColumnNames($tmp_schema  . $table,true,true);
	...
	function _connect($str,$user='',$pwd='',$db='',$ctype=0, $charset='')
	{
		...
		if(!empty($charset))
		{
			$rs = $this->Execute("SET CLIENT_ENCODING TO '". $charset ."'");
		}
	...
	function _nconnect($argHostname, $argUsername, $argPassword, $argDatabaseName,$ctype=0, $charset='')
	{
	 	return $this->_connect($argHostname, $argUsername, $argPassword, $argDatabaseName,-1, $charset);
	}
	...
	function _pconnect($str,$user='',$pwd='',$db='',$ctype=0,$charset='')
	{
		return $this->_connect($str,$user,$pwd,$db,1, $charset);
	}
