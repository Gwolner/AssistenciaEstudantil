...
class grid_aluno_rtf
{
   ...
   function gera_texto_tag()
   {
     global $nm_lang;
      ...
      { 
          ...
          if ($Cada_col == "nome_aluno" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              if (!NM_is_utf8($SC_Label))
              {
                  $SC_Label = sc_convert_encoding($SC_Label, "UTF-8", $_SESSION['scriptcase']['charset']);
   ...
   //----- nome_aluno
   function NM_export_nome_aluno()
   {
         $this->nome_aluno = html_entity_decode($this->nome_aluno, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->nome_aluno = strip_tags($this->nome_aluno);
         if (!NM_is_utf8($this->nome_aluno))
         {
             $this->nome_aluno = sc_convert_encoding($this->nome_aluno, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->nome_aluno = str_replace('<', '&lt;', $this->nome_aluno);
         $this->nome_aluno = str_replace('>', '&gt;', $this->nome_aluno);
         $this->Texto_tag .= "<td>" . $this->nome_aluno . "</td>\r\n";
   }
   //----- curso
   ...
   function monta_html()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aluno'][$path_doc_md5][0] = $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aluno'][$path_doc_md5][1] = $this->Tit_doc;
?>
